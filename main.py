...
class TrayApp:
    ...
    def open_detail_window(self) -> None:
        ...
        # show()/raise_/activateWindow first: update_result() only renders
        # when the window reports itself as actually on-screen (not
        # cloaked), and before show() it isn't visible at all yet.
    ...
    def _on_refresh_succeeded(self, result: RefreshResult) -> None:
        ...
        # update_result() self-guards against rendering while the window
        # is cloaked (e.g. parked on an inactive virtual desktop), so it's
        # safe to call unconditionally whenever the window object exists.
